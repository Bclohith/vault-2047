  ...
  return (
    <div className="flex flex-col min-h-screen bg-[#040b0e]">
      ...
      <main className="flex-grow flex flex-col items-center justify-start w-full">
        ...
        {/* Sub-Navigation Bar */}
        <div className="w-full px-6 py-3 bg-gray-950 flex flex-col md:flex-row justify-between items-center overflow-hidden border-b border-white/5 z-20 relative pt-24 md:pt-32">
          <div className="flex-1 flex flex-wrap justify-center items-center divide-x divide-white/10 w-full max-w-[1200px] mx-auto">
            {["VAULT2047 - VISION", "Vault Themes 2026", "Snapshot", "Speakers", "Call for speaker"].map((item, i) => (
              <div key={i} className={\`flex-1 px-4 md:px-6 py-2 flex justify-center items-center cursor-pointer hover:bg-white/5 transition-colors text-center \${i===0 ? 'border-b-4 border-[#B86A2E] bg-[#B86A2E]/10' : 'border-b-4 border-transparent'}\`}>
                <div 
                  className={\`text-xs md:text-sm font-semibold uppercase tracking-tight whitespace-nowrap \${i===0 ? 'text-white' : 'text-white/70'}\`}
        ...
        <div className="w-full relative h-[400px] md:h-[600px] bg-black flex flex-col justify-center items-center px-4 overflow-hidden">
          <Image 
            src="/assets/collaborate/hero-bg.jpg"
            alt="Agenda Hero Background"
            fill
        ...
        {/* Experience Section */}
        <div className="w-full px-6 md:px-28 py-24 bg-gray-950 flex flex-col justify-center items-center gap-12 overflow-hidden">
          <div className="text-center text-white text-3xl font-bold" style={{ fontFamily: "var(--font-orbitron)" }}>
            How You will Experience Vault2047
          </div>
          
          <div className="w-full overflow-x-auto hide-scrollbar pb-6">
            <div className="w-max flex justify-start items-stretch gap-6 mx-auto">
              {[
                ...
                <div key={idx} className="w-[300px] outline outline-1 outline-zinc-800 flex flex-col justify-start items-stretch bg-black group hover:outline-[#B86A2E] transition-all cursor-pointer">
                  <div className="w-full aspect-[4/3] relative">
                    <Image src={\`/assets/agenda/\${exp.img}\`} alt={exp.title} fill className="object-cover p-4" />
                  </div>
                  <div className="flex-1 p-6 relative bg-black/10 flex flex-col justify-start items-start gap-4 overflow-hidden">
                    <div className="w-[200px] aspect-square absolute bottom-[-50px] -left-10 opacity-30 group-hover:opacity-50 bg-[#0D6665] rounded-full blur-[60px] transition-opacity duration-700" />
                    <div className="text-white text-xl font-bold relative z-10" style={{ fontFamily: "var(--font-orbitron)" }}>{exp.title}</div>
                    <div className="text-neutral-400 text-sm font-normal relative z-10" style={{ fontFamily: "var(--font-ibm)" }}>{exp.desc}</div>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="w-full max-w-[1200px] flex flex-col items-center gap-12">
            <div className="w-full bg-black/10 flex flex-col border border-gray-900">
              {['Participate in strategic discussions', 'Engage in policy roundtables', 'Showcase solutions and innovations', 'Connect with global cyber stakeholders'].map((item, idx) => (
                <div key={idx} className={\`w-full py-6 px-4 \${idx !== 3 ? 'border-b border-gray-900' : ''} text-center text-white text-lg font-normal\`} style={{ fontFamily: "var(--font-ibm)" }}>
                  {item}
                </div>
              ))}
            </div>
...
fs.writeFileSync('src/app/agenda/page.js', content);
console.log('Successfully generated src/app/agenda/page.js');
